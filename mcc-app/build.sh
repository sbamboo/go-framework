...
# --- Configuration ---
PUBLIC_KEY_FILE="./libfwlink/signing/public.pem"
PRIVATE_KEY_FILE="./libfwlink/signing/private.pem"
OUTPUT_PATH="./builds"
APP_NAME="goframework_app" # Default app name, can be overridden
...
# Double check t
if [ -z "$T" ]; then
    echo "Error: Parameter -t must be set to either 'tui' or 'gui'." >&2
    exit 1
fi
...
case "$(echo "$T" | tr '[:upper:]' '[:lower:]')" in
    ...
    "gui")
        ...
        ;;
    *)
        echo "Error: Invalid value for -t. Must be 'tui' or 'gui'." >&2
        exit 1
        ;;
esac
...
mkdir -p "./libfwlink" || error "Failed to create libfwlink directory."
mkdir -p "./libfwlink/signing" || error "Failed to create libfwlink/signing directory."
...
LDFLAGS="-X 'mcc-app/libfwlink.AppVersion=$SEMVER' -X 'mcc-app/libfwlink.AppUIND=$UIND' -X 'mcc-app/libfwlink.AppChannel=$CHANNEL' -X 'mcc-app/libfwlink.AppBuildTime=$BUILD_TIME' -X 'mcc-app/libfwlink.AppCommitHash=$COMMIT_HASH' -X 'mcc-app/libfwlink.AppDeployURL=$DEPLOY_URL'"

# if $GH_UP_META_REPO is set and not "", add it to ldflags
if [[ -n "$GH_UP_META_REPO" ]]; then
    LDFLAGS+=" -X 'mcc-app/libfwlink.AppGithubRepo=$GH_UP_META_REPO'"
fi

# if $DEBUGGER_ADRESS is set and not "", add it to ldflags
if [[ -n "$DEBUGGER_ADRESS" ]]; then
    LDFLAGS+=" -X 'mcc-app/libfwlink.DebuggerHost=$DEBUGGER_ADRESS'"
fi
