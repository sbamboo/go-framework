...
param (
    ...
    [string]$appName = "goframework_app", # Default app name, can be overridden
    [switch]$help
)

# --- Configuration ---
$publicKeyFile = "./libfwlink/signing/public.pem"
$privateKeyFile = "./libfwlink/signing/private.pem"
$outputPath = "./builds"
...
if ([string]::IsNullOrWhiteSpace($t)) {
    Write-Error "Parameter -t must be set to either 'tui' or 'gui'."
    exit 1
}
...
switch ($t.ToLower()) {
    ...
    "gui" { $SUBTYPE_TAG = "gui" }
    default {
        Write-Error "Invalid value for -t. Must be 'tui' or 'gui'."
        exit 1
    }
}
...
if (-not (Test-Path "./libfwlink")) {
    New-Item -ItemType Directory -Path "./libfwlink" | Out-Null
}
if (-not (Test-Path "./libfwlink/signing")) {
    New-Item -ItemType Directory -Path "./libfwlink/signing" | Out-Null
}
...
$ldFlags = "-X 'mcc-app/libfwlink.AppVersion=$semver' -X 'mcc-app/libfwlink.AppUIND=$uind' -X 'mcc-app/libfwlink.AppChannel=$channel' -X 'mcc-app/libfwlink.AppBuildTime=$buildTime' -X 'mcc-app/libfwlink.AppCommitHash=$commitHash' -X 'mcc-app/libfwlink.AppDeployURL=$deployURL'"

# if $ghUpMetaRepo is set and not "", add it to ldflags
if ($ghUpMetaRepo -and $ghUpMetaRepo -ne "") {
    $ldFlags += " -X 'mcc-app/libfwlink.AppGithubRepo=$ghUpMetaRepo'"
}

# if $debuggerAdress is set and not "", add it to ldflags
if ($debuggerAdress -and $debuggerAdress -ne "") {
    $ldFlags += " -X 'mcc-app/libfwlink.DebuggerHost=$debuggerAdress'"
}
